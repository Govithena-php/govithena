...
class SearchGig extends Model
{
    ...
    {
        try {
            $sql = "SELECT gig.gigId, gig.farmerId, gig.title, gig.description, gig.category, gig.thumbnail, gig.capital, gig.profitMargin, gig.location, gig.timePeriod, user.firstName, user.lastName FROM gig INNER JOIN user ON gig.farmerId = user.uid WHERE (title LIKE :key OR description LIKE :key OR category LIKE :key OR location LIKE :key ) ";
